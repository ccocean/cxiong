...
综合三种信号来判断该切什么画面
*/
int strategy_status_analyze(Strategy_Result_t* stra, Strategy_CamControl_t *tch_cam, Strategy_CamControl_t *stu_cam)//根据三个不同的信号分析当前的画面决策
{
...
*/
static int switch_strategy(Track_Strategy_t *track_stra)//还需要控制摄像机的结构体和VGA信号的结构体
{
	if (track_stra->stra->result_activity == 1)
	{
		if (track_stra->stra->tch_que->size<FPS - 1)
		{
			EnQueue(track_stra->stra->tch_que, track_stra->tch->status);//持续向队列加入教师状态去统计

			//首先判断VGA信号
			vga_strategy_analyze(track_stra->stra, track_stra->vga);
			//其次响应学生的信号
			stu_strategy_analyze(track_stra->stra, track_stra->stu, track_stra->stu_cam);
                        int res=-1;
			//组合VGA信号和学生信号以及上一次教师的信号来判断该切怎样的画面
			if (track_stra->stra->TCH_flag>0)
			{
				res = strategy_status_analyze(track_stra->stra,track_stra->tch_cam,track_stra->stu_cam);
                                return (res > NULL_ACTIVITY) ? res : NULL_ACTIVITY;
			}
			else
			{
				track_stra->stra->status = SWITCH_PTEA;//刚启动时保持教师全景
                                return track_stra->stra->status;
			}	
		}
		else
		{
			...
			//int res_tch = -1;
                        int res=-1;
			EnQueue(track_stra->stra->tch_que, track_stra->tch->status);
			if (-1 < strategy_frame_analyze(track_stra->stra))
			{
				//先判断VGA信号
				vga_strategy_analyze(track_stra->stra, track_stra->vga);
				//判断学生的信号类型
				stu_strategy_analyze(track_stra->stra, track_stra->stu, track_stra->stu_cam);
                                //判断教师的信号类型
				tch_strategy_analyze(track_stra->stra, track_stra->tch, track_stra->tch_cam);
			}
			...
			ClearQueue(track_stra->stra->tch_que);
			return (res > NULL_ACTIVITY) ? res : NULL_ACTIVITY;
		}
	}
	else
	{
		track_stra->stra->timer_stra.finish = gettime();
		track_stra->stra->timer_stra.deltaTime = track_stra->stra->timer_stra.finish - track_stra->stra->timer_stra.start;
		if (track_stra->stra->timer_stra.deltaTime<track_stra->stra->display_time)
		{
			track_stra->stra->status = track_stra->stra->firstpicture;
			return track_stra->stra->status;
		}
		else
		{
			track_stra->stra->result_activity = 1;
			clear_timer(&track_stra->stra->timer_stra);
			return NULL_ACTIVITY;
		}
	}
        return track_stra->stra->status;
...
{
        stra = (Strategy_Result_t*)(sizeof(Strategy_Result_t));
        stra->result_activity = 1;
        stra->display_time = 0;
...
int first_PicMode(MainEnc_FirstPicMode_t *fpm, Track_Strategy_t *track_stra)
{
	...
	{
		track_stra->stra->result_activity = 0;
		track_stra->stra->timer_stra.start = gettime();
		track_stra->stra->display_time = fpm->display_time;
                track_stra->stra->firstpicture = fpm->firstpicture;
		return 1;
	}
	else
		return -1;
...
int track_strategy(Track_Strategy_t *track_stra)
{
	...
	return switch_strategy(track_stra);
}
